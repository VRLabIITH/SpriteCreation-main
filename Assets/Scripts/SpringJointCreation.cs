...
public class SpringJointCreation : MonoBehaviour
{
    LineRenderer Line;
    ...
    public List<Vector3> pointsOfSpringJoints;
    public List<HingeJoint2D> joints;
    public Dictionary<Vector3, SpringJoint2D> springJointDict;

    // Start is called before the first frame update
    void Start()
    {
        pointsOfSpringJoints = new List<Vector3>();
        listOfSelectedObjects = new List<Collider2D> ();
        springJointDict = new Dictionary<Vector3, SpringJoint2D>();
    ...
    Vector3 IntersectionPoint(LineRenderer linkLine)
    {
        ...
        if (denominator != 0)
        {
            ...
            springConnectedAnchor = new Vector3(x_intersect, y_intersect, -1f);

        }
        return springConnectedAnchor;
    }
    void CheckCollisionForSpring()
    {
        ...
        {
            //Debug.Log("1 Object");
            SpringJoint2D groundjoint = ground.AddComponent<SpringJoint2D>();
            ...
            pointsOfSpringJoints.Add(Line.GetPosition(Line.positionCount - 1));
            springJointDict.Add(Line.GetPosition(Line.positionCount -1), groundjoint);
        }
        if (listOfSelectedObjects.Count == 2)
        {
            Debug.Log("2 Object");
            SpringJoint2D sj1 = listOfSelectedObjects[0].gameObject.AddComponent<SpringJoint2D>();
            ...
            springJointDict.Add(IntersectionPoint(listOfSelectedObjects[0].GetComponent<LineRenderer>()), sj1);
            pointsOfSpringJoints.Add(IntersectionPoint(listOfSelectedObjects[0].GetComponent<LineRenderer>()));
        }
        //if (listOfSelectedObjects.Count == 2)
        //{
        ...
        //    //joints.Add(hj1);
        //    pointsOfJoints.Add(new Vector3(anchorPoint.x, anchorPoint.y, -1f));
